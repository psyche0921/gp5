...
- 파라미터 값 자체를 기기에서 읽어오는 것(`dst, amp, cab, eq, mod, dly, rvb` 7블록)은 여전히
  라이브 BLE HCI 캡처로 리버스엔지니어링한 것이라, 공식 문서가 아닙니다. `nr, pre, ns`(NAM)
  3블록은 patch_data의 어느 파트에 값이 들어있는지 아직 못 찾아서 이펙트 기본값으로 표시됩니다.
