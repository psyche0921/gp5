...
## 알려진 제한사항
- 패치 데이터 노티피케이션의 파트 2/3(각 100바이트, 실측 파라미터 값)는 라이브 BLE HCI 캡처로
  리버스엔지니어링해서 디코딩합니다 — `dst, amp, cab, eq, mod, dly, rvb` 7개 블록은 실제 저장된
  노브 값을 정확히 읽어옵니다. 다만 `nr, pre, ns`(NAM) 3개 블록은 이 파트 2/3 범위 밖(아마 파트
  0/1/4)에 있어서 아직 못 찾았고, 이 3개 블록은 여전히 이펙트 기본값으로 표시됩니다.
- 패치 저장(`save_patch` SysEx)도 공식 문서가 아니라 Valeton Suite의 실제 저장 동작을 캡처해서
