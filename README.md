...
## 알려진 제한사항
- 패치 데이터 노티피케이션 중 파트 3/4/최종 블록(개별 파라미터 실측값)의 정확한 바이트 오프셋은
  공개 프로토콜 문서에 없어 아직 디코딩하지 않습니다 — 즉, 패치를 불러오면 어떤 이펙트가 켜져
  있는지는 정확히 표시되지만, 그 이펙트의 노브 값 자체는 기기에서 읽어오지 못하고 이펙트 기본값으로
  표시됩니다. (노브를 움직여 값을 보내는 것 자체는 정상 동작합니다.)
